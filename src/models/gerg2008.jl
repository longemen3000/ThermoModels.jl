...
struct GERG2008{T} <: HelmholtzModel
    type::T
    syms::Vector{Symbol}
...
function GERG2008(xsel = collect(1:21))
    ...
    #original order
    iter4 = (findall(isequal(4), iter_ideal[xsel]))
    iter3 = (findall(isequal(3), iter_ideal[xsel]))
    iter2 = (findall(isequal(2), iter_ideal[xsel]))
    iter0 = (findall(isequal(0), iter_ideal[xsel]))
    ...
    return GERG2008(
        _type,
        N,
        acentric_factor[xsel],
        molecularWeight[xsel],
        criticalDensity[xsel],
        1 ./ criticalDensity[xsel],
        criticalTemperature[xsel],
        criticalPressure[xsel],
        ideal_iters,
        nr[:, xsel],
        zeta[:, xsel],
        n0ik[xsel],
...
function GERG2008(symsel::Vararg{Symbol})
    symvalues = GERG2008_symbols
    xsel = Vector{Int}()
    for i in symsel
        i in symvalues || throw(ArgumentError("invalid rank: $i"))
        push!(xsel, findall(isequal(i), symvalues)[1])

    end
    if iszero(length(xsel))
        return ArgumentError("no match found for input symbols")
    else
        return return GERG2008(xsel)
    end
end
...
function _f0(model::GERG2008{U}, rho, T, x) where U<:MaterialCompounds
    ...
    for i in model.ideal_iters[1]
        x[i] != x0 && begin
            ...
            ao2 =
                model.nr[4, i] * log(abs(sinh(model.zeta[1, i] * tau))) -
                model.nr[5, i] * log(cosh(model.zeta[2, i] * tau)) +
                model.nr[6, i] * log(abs(sinh(model.zeta[3, i] * tau))) -
                model.nr[7, i] * log(cosh(model.zeta[4, i] * tau))
...
function _delta(model::GERG2008{MaterialCompounds}, rho, T, x)
    rhor = inv(mixing_rule_asymetric(
        cubic_mean_rule,
...
function _tau(model::GERG2008{MaterialCompounds}, rho, T, x)
...
function _fr2(model::GERG2008{MaterialCompounds}, delta, tau, x)
    common_type = promote_type(typeof(delta), typeof(tau), eltype(x))
    res = zero(common_type)
    res0 = zero(common_type)
    x0 = zero(common_type)
...
function ideal_helmholtz_impl(mt::MultiVT,model::GERG2008, v, T, x)
    rho = 1.0e-3 / v
    R = Unitful.ustrip(Unitful.R)
    return R * T * _f0(model, rho, T, x)
end

function residual_helmholtz_impl(mt::MultiVT,model::GERG2008, v, T, x)
    rho = 1.0e-3 / v
    R = Unitful.ustrip(Unitful.R)
...
function mol_helmholtz_impl(mt::MultiVT,model::GERG2008{MaterialCompounds}, v, T, x)
    rho = 1.0e-3 / v
    R = RGAS
    delta = _delta(model, rho, T, x)
    tau = _tau(model, rho, T, x)
...
function mol_helmholtz_impl(mt::SingleVT,model::GERG2008{SingleComponent}, v, T)
    rho = 1.0e-3 / v
    R = RGAS
...
molecular_weight(model::GERG2008{MaterialCompounds}) = model.molecularWeight
molecular_weight(model::GERG2008{SingleComponent}) = only(model.molecularWeight)



#critical, single component:
mol_density(model::GERG2008{SingleComponent},st::CriticalPoint,unit=u"mol/(m^3)") = convert_unit(u"mol/L",unit,only(model.criticalDensity))
pressure(model::GERG2008{SingleComponent},st::CriticalPoint,unit=u"Pa") = convert_unit(u"Pa",unit,only(model.criticalPressure))
temperature(model::GERG2008{SingleComponent},st::CriticalPoint,unit=u"K") = convert_unit(u"K",unit,only(model.criticalTemperature))
mol_volume(model::GERG2008{SingleComponent},st::CriticalPoint,unit=u"m^3/mol") = convert_unit(u"m^3/mol",unit,only(model.criticalVolume))
...
mol_density(model::GERG2008{MaterialCompounds},st::CriticalPoint,unit=u"mol/(m^3)") = convert_unit.(u"mol/L",unit,model.criticalDensity)
pressure(model::GERG2008{MaterialCompounds},st::CriticalPoint,unit=u"Pa") = convert_unit.(u"Pa",unit,model.criticalPressure)
temperature(model::GERG2008{MaterialCompounds},st::CriticalPoint,unit=u"K") = convert_unit.(u"K",unit,model.criticalTemperature)
mol_volume(model::GERG2008{MaterialCompounds},st::CriticalPoint,unit=u"m^3/mol") = convert_unit.(u"m^3/mol",unit,model.criticalVolume)

covolumes(model::GERG2008{MaterialCompounds}) =
...
function Base.show(io::IO, sp::GERG2008{T}) where T <:MaterialCompounds
    ln = length(sp.syms)
